...
| [`patches/prefix-hit-and-kpool-tail/`](patches/prefix-hit-and-kpool-tail/README.md) | **In the recipe since 8 September 2026**, both knobs set by the production environment file (they were held for most of one night first, and that account is the page's most useful part). Two upstream backports the pinned vLLM predates: the hybrid prefix-cache annotation (an exact repeat at 8K goes 41.56 % → **83.12 %**, the ceiling, and follow-up TTFT −62 %) and the K-pool tail slot mapping (**99.67 % → 0.00 %** of tail writes addressing the wrong block). Promoted on two boots through the autostart unit with nine needle-lite runs at 6/6 and a cached-path equality test at 24/24. It cost one 495 s dump boot and a fresh ~53 GB-per-node sidecar. [`results/gates/prefix-hit-and-kpool-tail.md`](../../results/gates/prefix-hit-and-kpool-tail.md), [docs/11](../../docs/11-open-issues.md) §2.32–2.34 |
| [`patches/flashkda/`](patches/flashkda/README.md) | **In the recipe since 10 September 2026.** A port of vLLM [#55737](https://github.com/vllm-project/vllm/pull/55737): KDA chunked prefill through the fused `vllm._flashkda_C` kernel instead of the ten-kernel Triton chain, behind `HAREM_KDA_FLASHKDA=1`. Two matched sidecar-less boots one environment variable apart: sustained prefill **+6.5 %** on both repeats, TTFT at 7K **−5.1 %**, decode and KV pool unmoved, four gates full — and the measured gain is **1.6× the kernel share that predicts it**, which is open ([docs/11](../../docs/11-open-issues.md) §2.35). It cost one 394 s dump boot, a fresh 53 GiB-per-rank sidecar, and **18 minutes of downtime** to a preflight gate that demanded a sidecar in the mode that creates it ([docs/14](../../docs/14-troubleshooting.md) §10.6). [`results/gates/flashkda-ab-10sep.md`](../../results/gates/flashkda-ab-10sep.md) |
