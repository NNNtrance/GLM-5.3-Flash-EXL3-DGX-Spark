...
| [`scripts/`](../scripts/) | **Both launchers** — `start-tp3.sh` and `start-tp2full.sh`. They are per track but they live together, because the benchmark harness, the probes and the prompt sets beside them are identical at either rank count and a launcher is read next to them. The `*-prelude.sh` files here are **earlier arms kept as history**; the prelude each track runs is inside its own `patches/` tree, where the sidecar identity hashes it |
