...
| **TP=2 candidate D** (candidate C **plus** the vision tower and both backports) | KV pool **2,692,857 → 2,585,714, −4.0 %**, and a same-tree A/B on the two knobs (2,592,857 vs 2,585,714, −0.28 %) attributes that to the tower. C1 **59.45**/64.45, C8 **155.47**, fresh prefill **1,413**, TTFT 0.375/1.080, acceptance 60.3/62.6 — every row inside its band against candidate C. Gates probe 10/10, code 12/12 first try, tool-call 8/8, needle-lite **6/6 ×3**; six vision gates including **4 images + 2 videos in one request** (5,991 prompt tokens) and a third video refused HTTP 400 with the engine alive. Cached-path equality **24/24 byte-identical and correct** at ~11K/~83K/~177K, mean repeat hit 91.7 %. Prefix-cache block **4,608 tokens** at two ranks (gcd of the observed hits), and at that granularity an 8K exact repeat reads **0.0 %** with the knobs off against **57.54 %** (the ceiling) with them on, repeat TTFT 5.60 → 2.55 s. K-pool soak 4×4,096 all coherent; worst-case host memory on rank 0 during a 4-image+2-video request inside a C8 sweep: floor **4.90 GiB**, swap flat, **zero pages in or out**, text arm 141.03 vs 155.47 (−9.3 %, queue contention, named). Boot **280 s** through the two-node unit; dump boot 1,013 s; sidecar 78 → **79 GB**/rank. **One boot**, so the pool row is a cost and not noise | **two** nodes, TP=2, EP off, `exl3-zeus:754421f`, 0.85, `HAREM_VISION=1` + `--mm-encoder-tp-mode data`, `HAREM_PREFIX_HIT=1`, `HAREM_KPOOL_TAIL_FIX=1`, fast-load | 8 Sep 2026 | `results/gates/tp2-candidate-d.md`, `docs/15` §5.10, `docs/19` |
...
The count is **thirty-eight**, and [docs/11](../docs/11-open-issues.md) §1's opening paragraph defines
it: the 32 rows of that page's audit table plus the six withdrawn findings that are not rows of it.
The rows below are the ones with a story worth telling, not the whole 38.
...
| 1.14 | **"The vision tower cannot run at three ranks", plus "1.05 GiB of BF16 vision weights wasted on every rank at TP=2" and "`--mm-encoder-tp-mode data` would keep carrying it"** — in docs/03, docs/14, the README quick start and the TP=3 environment template | **All three withdrawn, by running it.** Production configuration 13 serves **4 images + 2 videos per request** at TP=3 with the text gates unmoved and no measurable cost. The 1.05 GiB is the *routed-experts-only* checkpoint; on the production full-scope checkpoint the tower is **6-bit EXL3, 0.557 GiB total / 0.416 GiB per rank**, readable from the safetensors headers in seconds. And the replication flag alone does **not** make the tower load: vLLM builds it with `quant_config=None` while this checkpoint's tower is quantized — the real obstacle, never in the sentence. [docs/18](../docs/18-vision-at-three-ranks.md) |
