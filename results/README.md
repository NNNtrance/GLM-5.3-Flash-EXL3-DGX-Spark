...
| `gates/quality-battery-production-13.md` | **The battery on the configuration this repository now recommends** — configuration 13 with both backports on — 8 September: the whole-cluster reboot it started from (`/health` at 321 s, a KV pool 0.6 % under the documented floor and said so), and **tool-eval-bench at reasoning effort `high`: 90.3 ±1.2 against 85.5 ±1.3 at `low` and 87.8 ±0.9 for the sibling**, with the proxy that raised the effort without touching the server, the per-category and per-scenario tables, the three regressions, and the token and latency price. |
