...
| **Two backports measured and not promoted** (8 September) | The hybrid prefix-cache annotation and the K-pool tail slot mapping, both real bugs, both fixed, **neither in the recipe**: an exact repeat at 8K goes 41.56 % → **83.12 %** (the ceiling) with follow-up TTFT **−62 %**, and 99.67 % of the indexer tail's writes stop addressing the wrong block. Held because one arm lost a gate item on one boot and six later runs did not reproduce it — and because the gate itself then flaked on unpatched production. Production is unchanged. [`tracks/tp3/patches/prefix-hit-and-kpool-tail/`](tracks/tp3/patches/prefix-hit-and-kpool-tail/README.md), [`results/gates/prefix-hit-and-kpool-tail.md`](results/gates/prefix-hit-and-kpool-tail.md) `[measured-here]` |
