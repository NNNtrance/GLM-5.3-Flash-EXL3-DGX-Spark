...
| Prefill, fresh unseen ~8K prompts | **1,744** tok/s — per-request, `bench/prefill-fresh.py`. The **sustained** figure from the throughput stopwatch is **1,867** tok/s since 10 September (FlashKDA, row below); the two instruments are not interchangeable and [docs/10 §1](docs/10-results-and-roofline.md) prints all four prefill figures with their methods |
| Sustained aggregate under load, stopwatch, **vision enabled** (7 September) | decode **205** tok/s with 12 code requests submitted at once (8 running + 4 queued; one-second peaks to 261) · prefill **1,914** tok/s with 24 fresh ~7K prompts in flight — 60 s counter deltas, nothing else on the cluster; [`results/speed/stopwatch-production-7sep.md`](results/speed/stopwatch-production-7sep.md) `[measured-here]` |
...
| Quality benchmarks, against the NVFP4 sibling recipe | GSM8K **97.5 %** (sibling 94.0), IFEval **80.0 % prompt / 86.0 % instruction** (78.9 / 85.1), tool-eval-bench **85.5 ±1.3** (**87.8 ±0.9** — behind, and it is four scenarios out of 88; the largest is a grader ordering rule, the template explanation was tested and refuted, and the build/checkpoint confound is still open), 6–7 September 2026 `[measured-here]`. The 1M needle and the full MMLU were **deferred on time** that night; the needle is now run (below) and the full MMLU is running. [`results/gates/quality-battery-production-12.md`](results/gates/quality-battery-production-12.md) | Full MMLU **85.30 ±0.29** on all 14,042 questions, 8 September ([§10](results/gates/quality-battery-production-13.md)) `[measured-here]`.
| **tool-eval-bench at reasoning effort `high`** (8 September) | **90.3 ±1.2** over three trials, `final_score` **91**, 160/176 points — against **85.5 ±1.3** for the same engine at `low` and **87.8 ±0.9** for the sibling at `low` (permutation p = 0.006 and 0.012). **The server was not touched**: the effort was raised by a 40-line proxy that injects `reasoning_effort` into the harness's requests ([`scripts/effort-proxy.py`](scripts/effort-proxy.py)), and production keeps serving at `low`. Three of the four scenarios behind the `low`-effort deficit go level or better; the price is **+90 % completion tokens** and +24 % median turn latency. Measured after a whole-cluster reboot into this configuration — `/health` at **321 s**, gates 10/10 · 12/12 · 8/8 · 6/6 · vision 5/5. [`results/gates/quality-battery-production-13.md`](results/gates/quality-battery-production-13.md) `[measured-here]` |
| **Needle in a haystack at 1M** (8 September) | **20/20, 100 %** — four haystack sizes to **997,952 tokens** by five depths, every cell retrieved; 1 h 25 min 32 s, 8.27M tokens. The NVFP4 sibling scored the same 20/20 in 5,288.6 s; this stack is **3.0 % quicker**. The test [`results/gates/quality-battery-production-12.md`](results/gates/quality-battery-production-12.md) deferred on time, now run on the vision-enabled configuration with both backports on. Head GPU 83.4 °C mean / 87 peak, no throttling, swap traffic zero. [`results/gates/quality-battery-production-13.md`](results/gates/quality-battery-production-13.md) §8 `[measured-here]` |
...
| **Two backports, promoted** (8 September) | The hybrid prefix-cache annotation and the K-pool tail slot mapping are now **both in the recipe**: an exact repeat at 8K goes 41.56 % → **83.12 %** (the ceiling) with follow-up TTFT **−62 %**, and 99.67 % of the indexer tail's writes stop addressing the wrong block. Promoted after two boots through the autostart unit with **nine needle-lite runs at 6/6**, three code exams at 12/12, and a cached-path equality test — 24 prompts at ~11K/~84K/~178K asked cold then repeated, **24/24 answers byte-identical and correct**, mean repeat hit 94.7 %. They were held for several hours first, and why is worth reading: [`tracks/tp3/patches/prefix-hit-and-kpool-tail/`](tracks/tp3/patches/prefix-hit-and-kpool-tail/README.md), [`results/gates/prefix-hit-and-kpool-tail.md`](results/gates/prefix-hit-and-kpool-tail.md) §6 `[measured-here]` |
| **FlashKDA for KDA prefill, promoted** (10 September) | The fused `vllm._flashkda_C` kernel replaces the ten-kernel Triton chain in KDA chunked prefill, a port of vLLM [#55737](https://github.com/vllm-project/vllm/pull/55737). Two matched sidecar-less boots one environment variable apart: sustained prefill **1,754 → 1,867 tok/s (+6.5 %, both repeats)**, TTFT at 7K **4.192 → 3.978 s**, decode +0.21 % and KV pool +0.31 % — both inside their bands — probe 10/10, code exam 12/12 first run, needle-lite 6/6, vision PASS. Confirmed on the production fast-load path at **1,865.7** and **1,866.6** tok/s. The kernel is **2.55×** the Triton chain at the production shape and the share it replaces is 6.19 % of prefill GPU time, which predicts +3.5–4.1 %: **the measured +6.5 % is 1.6× that and we cannot account for the rest** ([docs/11](docs/11-open-issues.md) §2.35). It cost one 394 s dump boot, a fresh 53 GiB per-rank sidecar, and 18 minutes of downtime to a preflight gate of ours that demanded a sidecar in the mode that creates it ([docs/14](docs/14-troubleshooting.md) §10.6). [`tracks/tp3/patches/flashkda/`](tracks/tp3/patches/flashkda/README.md), [`results/gates/flashkda-ab-10sep.md`](results/gates/flashkda-ab-10sep.md) `[measured-here]` |
| **Fail-closed tool-call parser + two xgrammar backports, promoted** (11 September) | Raised by YuXiaoPan in [issue #7](https://github.com/NNNtrance/GLM-5.3-Flash-EXL3-TP3-3x-DGX-Spark/issues/7). **(a)** The pinned vLLM's `glm47_moe` parser *salvages* a malformed tool call instead of refusing it; the client echoes the salvage back, the model imitates its own corruption, and the session ends in empty turns reported as complete. The parser now buffers each call and validates it whole — name, argument keys, schema `properties`, **and `parameters.required`** — and surfaces a rejected call as plain content. **This changes a visible API behaviour**: a valid streaming tool call now arrives as one delta instead of a name delta plus argument deltas. **(b)** Backports of vLLM [#52805](https://github.com/vllm-project/vllm/pull/52805) and [#53046](https://github.com/vllm-project/vllm/pull/53046) stop XGrammar feeding a terminated grammar the speculative tokens that follow its stop token: `Failed to advance FSM ... Please file an issue.` went from **776 `ERROR` lines over 102 requests in one 11-hour boot — 100 % of the log's `ERROR` volume, every answer correct — to 0** over the same strict protocol, and `grammar_matcher` warnings 872 → 0. Gates with both in: probe **10/10** (content-only 9/9, empty-content requests **0**), code exam **12/12**, strict tool-call gate 4 × 30 at **120 turns / 132 calls / 132 well-formed / 0 rejected / 0 out-of-schema / 0 empty / 0 repeat**, KV pool **unchanged** at 7,063,360. Neither touches a compute path, so no speed number was taken `[not measured]`; needle-lite and vision were not re-run `[not tested]`. Cost: three boots and a fresh sidecar, because the sidecar identity is keyed on the patch set. [`tracks/tp3/patches/glm47-failclosed/`](tracks/tp3/patches/glm47-failclosed/README.md), [`tracks/tp3/patches/xgrammar-backports/`](tracks/tp3/patches/xgrammar-backports/README.md), [`results/gates/failclosed-11sep.md`](results/gates/failclosed-11sep.md), [`results/gates/xgrammar-backports-11sep.md`](results/gates/xgrammar-backports-11sep.md), [docs/14](docs/14-troubleshooting.md) §9.13–9.14 `[measured-here]` |

| **Sparse-attention top-k 2048 → 8192, promoted** (12 September) | The last finding of [issue #7](https://github.com/NNNtrance/GLM-5.3-Flash-EXL3-TP3-3x-DGX-Spark/issues/7), and the only change that moved it. Past ~32k tokens of agentic context the **string arguments of tool calls** rot while the prose stays fine — a character slipping inside a path (`depo` → `dego`), context spliced onto the end of a path, markup leaking into a value — and no single-turn gate can see it. A 30-turn replay at 30–41k prompt tokens, scored on path arguments, with one thing removed per three-node restart: speculative decoding off, FlashKDA off, bf16 KV instead of fp8 and the structural-tag grammar off **all left it in place**; `index_topk` 2048 → 4096 → 8192 took bad turns out of 30 from **11/7 to 6/4 to 2/3** (full prefill / prefix reuse), and the production boot's replay read **1/30**. `"index_topk":8192` now travels in `--hf-overrides`. Gates: probe **10/10**, code exam **12/12**, needle-lite **6/6**, strict tool-call gate **72 calls / 72 well-formed / 0 rejected / 0 corrupt turns** to 31k, KV pool **7,033,057** in band, boot 170 s. The same gate **without** `strict` left markup in 7 of 61 calls, so the grammar stays required. **What it cost, and it is the largest price here: a 7K-prompt prefill reading 1,868 → 1,366 tok/s (−27 %)**, per-turn prefill on the replay ≈ +11 %, decode and KV unmoved; the sweep was not re-run `[not tested]`. Two hard limits found: 16384 and 65536 die in worker init with `CUDA error: invalid argument`, and dense MLA (`index_topk: null`) is refused by the EXL3 attention backend. Whether the residual is the checkpoint's design limit or this engine's indexer precision is **open** ([docs/11](docs/11-open-issues.md) §2.36). [`results/gates/index-topk-8192-12sep.md`](results/gates/index-topk-8192-12sep.md), [docs/14](docs/14-troubleshooting.md) §9.15 `[measured-here]` |
...
ladder has never been derived — same harness and same protocol `[measured-here]`.
**Candidate D** since 8 September: candidate C ([docs/15 §5.9](docs/15-tp2-track.md), the
indexer-workspace bound at two ranks) **plus the vision tower and the two upstream backports**, with
the memory rung and every other setting untouched. The speed rows below are candidate D's, measured
in a different session from candidate C's and inside the bands:

| | |
|---|---|
| Single-stream decode (C1) | **59.45** tok/s aggregate (**64.45** per stream) — candidate C read 60.08 / 65.96 |
| Aggregate at 8 concurrent streams (C8) | **155.47** tok/s — candidate C read 157.71 |
| Prefill, fresh unseen ~8.4K prompts | **1,413** tok/s — candidate C read 1,414 |
| KV pool at `max_model_len` 1,000,000 | **2,585,714** tokens — about 2.6 concurrent 1M-token requests. Candidate C read 2,692,857; the tower costs **−4.0 %**, and with one boot each that is written down as a cost rather than as boot noise |
| **Multimodal: 4 images + 2 videos per request** | Six gates, all passing, including 4 images and 2 videos in **one** request (5,991 prompt tokens) and a third video correctly refused with HTTP 400 while the engine stays up. [docs/19](docs/19-vision-at-two-ranks.md) |
| **The two backports** | `HAREM_PREFIX_HIT=1` and `HAREM_KPOOL_TAIL_FIX=1`, the same night they were promoted at three ranks. An 8K exact repeat reads **57.5 % of its prompt out of the cache, which is 100 % of the ceiling** — and the ceiling is set by a **4,608-token** block at two ranks rather than the three-node 3,328, a number you measure rather than copy |
| Quality | correctness probe **10/10**, code exam **12/12** on the first attempt, tool-call **8/8**, needle-lite **6/6 three times**; MMLU sample (1,995 q) **86.02 ±0.75** is candidate B's, not re-run since |
| Cold boot, fast-load | **280 s** through the autostart unit (the one-off dump boot that writes the sidecar is 1,013 s; sidecar 79 GB per rank) |
| Autostart unit → `/health` 200 | **280 s**, `systemctl start` on both nodes, on the configuration this table describes. Installed **disabled** — exactly one of the two units may be enabled. **No reboot test yet** `[not tested]` |
...
[docs/14](docs/14-troubleshooting.md) indexes all 87 failures we hit by symptom with the exact log
...
ranks nothing needs padding, so it is genuinely *shorter*: the patch tree is eighteen files rather
than twenty-three, and four of the eighteen are the three-node track's files used byte for byte.
...
| Speed by category, C1, re-measured on **production 12** (7 September) | code **61.5** · math **76.2** · JSON **73.1** · prose **29.0** tok/s | code 61.7 · math 79.6 · JSON 72.8 · prose 29.1 (production 9, the row above) | acceptance 46 / 57 / 53 / **13 %**, unchanged. Every category inside its own round-to-round spread, so **+37.8 % of KV pool cost no category any tokens per second**. Prose is the slow row because the draft accepts one token in eight there, no flag changes that, and **the newest drafter revision does not change it either** — measured, same night: [docs/10 §1.2](docs/10-results-and-roofline.md), [`results/speed/category-speeds-production-12.md`](results/speed/category-speeds-production-12.md) `[measured-here]` |
...
15. [14 — Troubleshooting](docs/14-troubleshooting.md) — **all 87 failures we hit, indexed by symptom, with the exact log line.** A triage order at the top, and a ranked index of the twenty that produced no error message at all. If something is wrong right now, start here.
...
20. [19 — Vision at two ranks](docs/19-vision-at-two-ranks.md) — **the same tower on two nodes, measured 8 September 2026.** What divides at two ranks and why we replicated the tower anyway rather than slice a 6-bit EXL3 one; the anchor dependency that made the two-node patch tree grow a file it had deliberately refused; the drafter `config.json` carrying a three-node pad that stops a two-node boot four minutes in; the six gates, and what the tower cost in pool, boot and host memory.
21. [audit/](audit/README.md) — a post-install self-check with our own numbers beside each step, the provenance table for every headline figure, and the retraction index. Run `audit/run-audit.sh` before you conclude anything about your install.
22. [charts/](charts/) — four figures generated from the CSVs in [`results/`](results/README.md) by [`charts/make-charts.py`](charts/make-charts.py), standard library only, so you can regenerate them and check the bars against the rows.
23. [systemd](systemd/README.md) — **the autostart units and their preflights**, real, installed and — at three nodes — reboot-tested, plus the hazard that comes first: if you also run the NVFP4 sibling, or the other track's unit, whichever is enabled wins a reboot. Exactly one may be enabled. Read this before a reboot.
24. [tracks/](tracks/README.md) — **the files that differ between the two node counts**, one folder each: the environment template, the patch tree and the autostart unit. Everything else in this repository is shared, and this page says why each shared thing is shared. It also carries the one trap the move introduces: the directory name here is not the directory name on your nodes.
...
```text
 ...
 8. CONFIGURE.  Copy scripts/ and tracks/tp3/patches/ to ~/exl3-zeus/ on every node; hard-link
    ...
    Since 12 September the template's --hf-overrides carries TWO keys -- the sidecar's
    quantization_config.json AND "index_topk":8192, in ONE object with no spaces. Leave it
    at 8192 if you run agents: at the checkpoint's 2048 tool-call string arguments corrupt
    past ~32k of context, and the price is 27 % of a 7K-prompt prefill reading (docs/14
    section 9.15, results/gates/index-topk-8192-12sep.md). Changing it changes the fast-load
    identity, so it costs a dump boot -- budget it here, not after step 10.
